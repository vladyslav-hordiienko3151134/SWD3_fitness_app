  ...
  return (
    <div className="wrap" style={{ paddingTop: '4rem' }}>
      <h1 className="title" style={{ marginBottom: '3rem' }}>
        {isOrganizer ? <>Teaching <span className="neon">Schedule</span></> : <>My <span className="neon">Bookings</span></>}
      </h1>
      ...
      {loading ? (
        ...
        <div className="card" style={{ textAlign: 'center', padding: '4rem' }}>
          <p style={{ color: 'var(--text-secondary)', fontSize: '1.1rem' }}>
            {isOrganizer ? "You don't have any upcoming classes scheduled." : "You haven't booked any sessions yet."}
          </p>
        </div>
      ) : (
            ...
            <div key={booking.booking_id} className="card" style={{ display: 'flex', flexDirection: 'column' }}>
              <div style={{ marginBottom: '1.5rem' }}>
                ...
                <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
                  {isOrganizer ? `Class ID: #${booking.booking_id}` : `Booking ID: #${booking.booking_id}`}
                </p>
              </div>

              <div style={{ marginBottom: '2rem', fontSize: '0.9rem', flex: 1 }}>
                <div style={{ marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <span>📅</span> {new Date(booking.event_date).toLocaleDateString('en-GB')} at {booking.start_time?.substring(0, 5)}
                </div>
                <div style={{ marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <div style={{ marginTop: 'auto' }}>
                ...
                {isOrganizer && (
                  <Link href={`/events/edit?id=${booking.booking_id}`} className="btn-alt" style={{ display: 'block', textAlign: 'center', width: '100%', padding: '0.8rem' }}>
                    Edit Class Details
                  </Link>
                )}
              </div>
