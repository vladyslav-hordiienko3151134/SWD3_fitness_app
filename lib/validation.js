...
//phone validation
  if (!isUpdate || data.phone !== undefined) {
    ...
    } else if (!/^087\d{7}$/.test(phone)) {
      //must follow format 087XXXXXXX
      errors.phone = 'Phone number must follow the format 087XXXXXXX';
    }
  }
  ...
  if (!isUpdate || data.password !== undefined) {
    ...
    } else if (data.password && (typeof data.password !== 'string' || data.password.length < 6)) {
...
export function validateEvent(data, isUpdate = false) {
  ...
  if (!isUpdate || data.event_date !== undefined) {
    ...
    } else {const eventDate = data.event_date; // eventDate was not defined anywhere, data came as data.event_date but code tried to read eventDate which did not exist, that causing validation to crash {
